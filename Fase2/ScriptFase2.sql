...
BEGIN
    IF(UPPER(:NEW.TIPO)='PRINCIPAL') THEN
        ...
        IF(V_TECNICOS = 1) THEN
            RAISE_APPLICATION_ERROR(-20002,'Este equipo ya tiene un técnico principal');
        END IF;
    ...
    ELSE
        ...
        IF(V_TECNICOS_2 = 1) THEN
            RAISE_APPLICATION_ERROR(-20003,'Este equipo ya tiene un técnico asistente');
        END IF;
    END IF;
END;
...
VALUES ('53983064V','Alvaro','Muñoz','Ironfix',1500,
(SELECT ID_EQUIPO FROM EQUIPOS WHERE UPPER(NOMBRE)='IBAISITOS'),'PRINCIPAL');
...
INSERT INTO JUGADORES(DNI,NOMBRE,APELLIDOS,NICKNAME,ROL,DORSAL,SUELDO,ID_EQUIPO) 
VALUES ('45697647L','Tükek','Irfan Berk','Armut','TOP',1,1500,
