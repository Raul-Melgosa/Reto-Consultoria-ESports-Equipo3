...
CREATE OR REPLACE PROCEDURE PARTIDOS_GANADOS_EQUIPOS(id_equipo in equipos.id_equipo%TYPE, numero_ganados OUT NUMBER)
IS
BEGIN
...
CREATE OR REPLACE PROCEDURE GET_SALARIO_EQUIPO(p_id_equipo IN NUMBER,resultados OUT NUMBER)
IS
BEGIN
    SELECT (SUM(NVL(JUGADORES.SUELDO,0))+SUM(NVL(TECNICOS.SUELDO,0)))*14 SUMA_SUELDOS into resultados
    FROM JUGADORES, TECNICOS
    WHERE TECNICOS.ID_EQUIPO=JUGADORES.ID_EQUIPO
    AND TECNICOS.ID_EQUIPO=p_id_equipo
